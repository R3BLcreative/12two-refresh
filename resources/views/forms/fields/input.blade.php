...
<div @isset($class)class="{{ $class }}" @endisset>
	<label for="{{ $id }}" class="block mb-2 font-bold @empty($label) invisible @endempty @error($id, $bag) text-error-100 @enderror">{{ $label ?? $desc }} @isset($required)<span class="text-red-700 font-black">*</span>@endisset</label>
