...
<div class="{{ $class }} flex flex-col gap-4" @isset($details) data-hasDeps="1" @endisset>
	...
	<div id="{{ $id }}-details-wrap" class="flex flex-col gap-4 w-full @if($showDetails) max-h-[500px] opacity-1 @else max-h-0 opacity-0 @endif transition-all ease-in-out duration-500 overflow-hidden" @if(old($id.'_'.$i))aria-expanded="true" @else aria-expanded="false" @endif>
		<div class="">
			<label for="{{ $id }}-details" class="block font-bold @error($id.'-details', $bag) text-error-100 @enderror">Explanation <span class="text-red-700 font-black">*</span></label>
		...
		<textarea id="{{ $id }}-details" name="{{ $id }}-details" placeholder="Explain here..." class="w-full @error($id.'-details', $bag) border-error-100 @enderror" rows="4" @isset ($detailsMax)data-max="{{ $detailsMax }}" data-counter="{{ $id }}-details-counter" @endisset>{{ old($id.'-details') }}</textarea>
		@isset($detailsMax)
