...
<form id="subscribe-form" action="{{ route('subscribe.store') }}#subscribe" class="{{ $class }}" method="post" enctype="multipart/form-data" novalidate>
	...
	<x-fields::input class="laptop:col-span-full" id="email_confirmation" type="email" placeholder="john@email.com" label="Confirm Email" value="{{ old('email_confirmation') }}" required="1" bag="{{ $bag }}" />
